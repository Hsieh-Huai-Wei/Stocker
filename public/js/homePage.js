// 設定畫圖的width, height
var margin = { top: 20, right: 50, bottom: 30, left: 58 },
...
// 設定文字區域
var textSvg = d3.select("#select").append("svg")
...
function renderData(data) {
  ...
  for (let i=0; i<data.length ; i++){
    let index = {};
    index.date = data[i].date;
    index.volume = data[i].volume;
    newData.push(index)
  }
...
function draw(data, volumeData) {
  ...
  svg.select("g.sma.ma-1").attr("clip-path", "url(#candlestickClip)").datum(techan.indicator.sma().period(20)(data)).call(sma1);
  svg.select("g.ema.ma-2").attr("clip-path", "url(#candlestickClip)").datum(techan.indicator.ema().period(50)(data)).call(ema2);
...
function move(coords, index) {
  ...
  for (i = 0; i < dataArr.length; i++) {
    if (coords.x === dataArr[i].date) {
      svgText.text( dataArr[0].name + " " + d3.timeFormat("%Y/%m/%d")(coords.x) + " , 開盤：" + dataArr[i].open + " , 高：" + dataArr[i].high + " , 低：" + dataArr[i].low + " , 收盤： " + dataArr[i].close + " , 漲跌： " + dataArr[i].change + " ( " + dataArr[i].percentChange + "% ) " + " , 成交量：" + dataArr[i].volume);
